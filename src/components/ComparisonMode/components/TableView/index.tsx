  ...
  const renderTable = (section: any) => {
    ...
    section.content.forEach((metric: any, id: any) => {
      let columnValues = comparison.map((ft, idx) => {
        // @ts-ignore
        let rawValue = geosesData[ft?.properties.CD_MUN][metric.id] && (
          <MetricDetails key={id} feature={ft} metric={metric} />
        );
    ...
    <div className="table-view">
      <div className="table-header" style={{ width: tableWidth }}>
        ...
        <div
          ...
          style={{ width: `${dataColumnWidth * 4}px` }}
        >
          {columns.map((col) => (
            <div
              key={col.dataKey}
              className="dataColumn"
              style={{ flex: `0 0 ${dataColumnWidth}px` }}
            >
              {col.label}
            </div>
          ))}
